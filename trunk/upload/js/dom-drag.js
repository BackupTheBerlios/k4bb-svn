 ...
 **************************************************
 * USED IPB version with ability to constrain the
 * dragged object to the current window
 **************************************************
 * k4 Bulletin Board Changes
 * Added a function to handle clicking on a button
 * to show the object to drag and call the Drag.init
 **************************************************/
...
var Drag = {
	...
	click_and_show : function(opener_id, o_id) {
		var d		= new k4lib();

		var opener	= d.getElementById(opener_id);
		var to_drag = d.getElementById(o_id);
		
		if(opener && to_drag) {
			opener.onmousedown = function() {
				
				to_drag.style.display	= 'block';
				to_drag.style.position	= 'absolute';
				to_drag.style.zIndex	= 1000;
				to_drag.style.left		= parseInt(d.left(opener) - 5) + "px";
				to_drag.style.top		= parseInt(d.top(opener) - 5) + "px";
				Drag.init(to_drag, null);
